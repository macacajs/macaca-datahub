...
describe('test/app/service/database.js', () => {
  ...
  it('exportData', async () => {
    ...
    assert(await isDir([ 'one', 'one_GET_one' ]));
    assert(await isDir([ 'one', 'one_GET_one', 'scene' ]));
    assert(await isDir([ 'one', 'one_GET_one', 'schema' ]));
    assert(await isFile([ 'one.json' ]));
    assert(await isFile([ 'one', 'one_GET_one', 'scene', 'default.json' ]));
    assert(await isFile([ 'one', 'one_GET_one', 'schema', 'request.json' ]));
    ...
    const sceneContent = JSON.parse(await fs.readFile(path.join(exportDir, 'one', 'one_GET_one', 'scene', 'default.json'), 'utf8'));
    ...
    const schemaContent = JSON.parse(await fs.readFile(path.join(exportDir, 'one', 'one_GET_one', 'schema', 'request.json'), 'utf8'));
